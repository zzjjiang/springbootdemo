	...
	function navBar(strData) {
		...
		var getChildrenHtml = function (item) {
			...
			if (item.children != undefined && item.children.length > 0) {
				ulHtml += '<a title="' + item.title + '" data-layid="' + getGuidByMid(item.id, item.title) + '" href="javascript:void(0)">';
				...
				var href = item.href + "?modelId=" + item.id;
				//target : '_blank' '_self' '_top'
				if (item.isTab == 1) {
					ulHtml += '<a title="' + item.title + '" data-layid="' + getGuidByMid(item.id, item.title) + '" href="' + href + '" data-url="' + href + '" target="_blank">';
				} else {
					ulHtml += '<a title="' + item.title + '" data-layid="' + getGuidByMid(item.id, item.title) + '" href="' + href + '" data-url="' + href + '">';
				}
				ulHtml += convertIconHtml(item.icon);
